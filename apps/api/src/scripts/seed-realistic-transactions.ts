...
function getWeightedRandomStatus(weights: Record<string, number>): string {
...
async function updateUserBalances(queryRunner: any, users: any[]) {
  ...
  for (const user of users) {
    ...
    for (const tx of transactions) {
      if (tx.coins_earned) totalEarned += tx.coins_earned
      if (tx.coins_redeemed) totalRedeemed += tx.coins_redeemed
    }
